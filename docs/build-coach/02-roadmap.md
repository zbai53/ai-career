...
- [x] **Day 33:** Spring Boot endpoint refinements and validation polish. Added `POST /api/interview/{id}/end-with-review` (combined summary + coach review). Created `InterviewService` to encapsulate agent calls + DB persistence; refactored `InterviewController` to delegate. All 225 tests passing (211 Python + 14 Spring Boot).
- [x] **Day 34:** Created `app/rag/ats_keywords.py`: 3 industries × 8 roles × ~125 keywords. `find_missing_keywords()` checks resume raw text with case-insensitive substring match, returns `{present, missing, coverage_percent}`. Integrated into `MatchAgent.match()` via `_infer_ats_role()` title → role key mapping. `MatchResult` now includes `ats_present`, `ats_missing`, `ats_coverage_percent`. Documented `interview-eval-v1.md` with STAR analysis, Q&A examples, talking points.
- [x] **Day 35:** Full end-to-end smoke test: all 211 Python tests passing, all 4 Docker containers healthy, both services running. Smoke tests confirmed: 6/6 endpoints return expected responses. Updated `architecture-workflow.md` with RAG pipeline, interview flow, and coach review sections. Comprehensive `api-endpoints.md` audit — added 9 previously undocumented endpoints. Phase 5 complete.
